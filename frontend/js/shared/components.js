...
const Components = {
    ...
    loadUserData() {
        ...
        nameElements.forEach(el => {
            if (el && displayName) {
                el.textContent = displayName.split(' ')[0]; // First name only for welcome
            }
        });
        ...
        if (profileName && user.username) profileName.value = user.username;
        ...
        if (profileClassDisplay && user.user_class) profileClassDisplay.textContent = user.user_class;
